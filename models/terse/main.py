...
def main():
    ...
    g_gan_loss_meter, d_real_loss_meter, d_fake_loss_meter = AverageMeter(), AverageMeter(), AverageMeter()
    ...
    for epoch in range(start_ep, opt.n_epochs):
        for i, (indices, bg_img_feats, fg_img_feats, fg_msk_feats, fg_bboxes, comp_img_feats, comp_msk_feats, comp_crop_feats, labels, trans_labels, catnms) in enumerate(train_loader):
            model.start_train()
            g_gan_loss, d_real_loss, d_fake_loss = model.train_disc_gen(bg_img_feats, fg_img_feats, fg_msk_feats, fg_bboxes, comp_img_feats, comp_msk_feats, labels)
            ...
            if (epoch * len(train_loader) + i) % 10 == 0:
                logger.info(
                    "[Epoch %d/%d] [Batch %d/%d] [G - gan: %.3f] [D - real: %.3f, fake: %.3f]"
                    % (epoch + 1, opt.n_epochs, i + 1, len(train_loader), g_gan_loss_meter.avg, d_real_loss_meter.avg, d_fake_loss_meter.avg)
                )

                ##### Save loss values to CSV
                save_loss_to_csv_terse(loss_csv_file_path, epoch+1, i+1,
                                 g_gan_loss_meter.avg, d_real_loss_meter.avg, d_fake_loss_meter.avg)
