    ...
    for epoch in range(start_ep, opt.n_epochs):
        for i, (indices, bg_img_feats, fg_img_feats, fg_msk_feats, comp_img_feats, comp_msk_feats, comp_bboxes, labels, catnms) in enumerate(train_loader):
            ...
            if (epoch * len(train_loader) + i) % 10 == 0:
                logger.info(
                    "[Epoch %d/%d] [Batch %d/%d] [G - gan: %.3f ndiv: %.5f] [D - real: %.3f, fake: %.3f]"
                ...
                save_loss_to_csv_placenet(loss_csv_file_path, epoch+1, i+1,
                                 g_gan_loss_meter.avg, g_ndiv_loss_meter.avg, d_real_loss_meter.avg, d_fake_loss_meter.avg)
